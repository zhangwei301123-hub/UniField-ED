...
# ================= 2. 稠密电子密度场 (PointNext 等) =================
        ...
        train_dataset = ED5OEDenseDataset(pkl_path=pkl_path, split='train', grid_size=grid_size, targets=targets)
        val_dataset = ED5OEDenseDataset(pkl_path=pkl_path, split='valid', grid_size=grid_size, targets=targets)
    ...
    # ================= 3. 混合融合表征 =================
        ...
        train_dataset = ED5OEDualDataset(
            pkl_path=pkl_path, split='train', 
            grid_size=grid_size, max_radius=max_radius, targets=targets
        )
        val_dataset = ED5OEDualDataset(
            pkl_path=pkl_path, split='valid', 
            grid_size=grid_size, max_radius=max_radius, targets=targets
        )
    ...
    # ================= 4. 纯原子图结构 (ViSNet, EquiformerV2) =================
        ...
        val_dataset = ED5OEAtomisticDataset(pkl_path=pkl_path, split='valid', targets=targets)
        collate_fn = atomistic_collate_fn
