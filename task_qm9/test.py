...
def build_test_dataset(data_config):
    """
    专门为 QM9 测试集准备的工厂函数，动态读取 dataset_mode 并将 split 设为 'test'
    """
    ...
    if mode == "repr_ed_field":
        from dataset_qm9_edpointcloud import QM9EDPointCloudDataset, ptv3_collate_fn
        test_dataset = QM9EDPointCloudDataset(pkl_path=pkl_path, split='test', grid_size=grid_size, targets=targets)
        ...
        test_dataset = QM9DenseDataset(pkl_path=pkl_path, split='test', grid_size=grid_size, targets=targets)
        ...
        test_dataset = QM9DualDataset(pkl_path=pkl_path, split='test', grid_size=grid_size, max_radius=max_radius, targets=targets)
        return test_dataset, qm9_dual_collate_fn

    elif mode == "repr_atomistic_graph":
        from dataset_qm9_atomistic_graph import QM9AtomisticDataset, qm9_atomistic_collate_fn
        test_dataset = QM9AtomisticDataset(pkl_path=pkl_path, split='test', targets=targets)
        return test_dataset, qm9_atomistic_collate_fn
    # ====================================================================
...
def main():
    ...
    # ================== 3. 构建模型与恢复权重 ==================
    # 💡 极其重要的修复：先读取 checkpoint 拿到 normalizer，再初始化模型！
    checkpoint = torch.load(ckpt_path, map_location=device)
    normalizer = checkpoint.get('normalizer', None)
    
    # 这样 UniFieldNet 在初始化时就能正确拿到 mean 和 std 啦！
    model, regressor = build_model(model_config, output_dim=output_dim, normalizer=normalizer)
