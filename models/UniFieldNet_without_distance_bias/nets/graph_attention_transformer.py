...
_MAX_ATOM_TYPE = 100         
_AVG_NUM_NODES = 19.0        
_AVG_DEGREE = 16.0            # 在 cutoff=5.0 下的近似平均邻居数
...
class GraphAttentionTransformer(torch.nn.Module):
    ...
    def forward(self, f_in, pos, batch, node_atom, **kwargs) -> torch.Tensor:
        ...
        # 确保传入的原子序数不会越界，并且是非负数 (因为 one_hot 不接受负数)
        node_atom = torch.clamp(node_atom, min=0, max=_MAX_ATOM_TYPE - 1)
        # ==============================================================================
        ...
        # ================== 💡 关键补丁：半路拦截原子特征 ==================
        # 从 kwargs 里提取 return_node_feats 参数，如果为 True，就直接返回微观节点特征！
        if kwargs.get('return_node_feats', False):
            return node_features
        # =================================================================
