...
class EquivariantScalar(OutputModel):
    ...
    def __init__(self, hidden_channels, activation="silu", allow_prior_model=True):
        ...
        self.reset_parameters()
   '''
    # [修改 1] 在 __init__ 参数中增加 output_dim，默认为 1 以保持兼容性
    def __init__(self, hidden_channels, activation="silu", allow_prior_model=True, output_dim=1):
        super(EquivariantScalar, self).__init__(allow_prior_model=allow_prior_model)
        ...
        self.output_network = nn.ModuleList([
                ...
                GatedEquivariantBlock(
                    hidden_channels // 2, 
                    output_dim,  # [修改 2] 将原来的 1 替换为变量 output_dim
                    activation=activation,
                    scalar_activation=False,
                ),
        ])
