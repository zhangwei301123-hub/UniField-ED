...
class CloudToAtomInteraction(nn.Module):
    def __init__(self, atom_dim, cloud_dim, interaction_radius=4.0, num_heads=4):
        ...
        self.rbf_expansion = GaussianSmearing(start=0.0, stop=interaction_radius, num_gaussians=self.num_gaussians)
        
        self.dist_mlp = nn.Sequential(
            nn.Linear(self.num_gaussians, 64),
            nn.SiLU(),
            nn.Linear(64, 1)
        )
...
class ED5UniFieldNet(nn.Module):
    def __init__(self, config, output_dim=7, normalizer=None):
        ...
        # 💡 [兼容设定]：虽然前向传播不再反归一化，但仍然把 mean 和 std 挂载在模型里
        # 这样可以在推理或测试脚本中，直接调用 model.std 和 model.mean 进行评估换算
        ...
        # 💡 [防报错必加]：安全弹出 output_dim，防止底层 Equiformer 报错
        ...
        # 💡 [防降维打击]：用自定义投影层平滑过渡 (1024 -> 64)，避开 Equiformer 的 1D 瓶颈
    ...
    def forward(self, input_dict):
        ...
        # 确保 Equiformer 的 forward 有 return_node_feats 拦截逻辑
        ...
        # =========================================================
        # 💡 [核心修复]：这里必须输出 Normalized 的值 (N(0,1))
        # 你的 engine.py 会在这个归一化空间中算 MSE Loss (< 1)
        # 所以删除了 out = out * self.std + self.mean
        # =========================================================
